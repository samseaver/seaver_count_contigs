package seaver_count_contigs::seaver_count_contigsClient;
...
seaver_count_contigs::seaver_count_contigsClient

=head1 DESCRIPTION


A KBase module: seaver_count_contigs
...
sub new
{
    ...
    my $self = {
	client => seaver_count_contigs::seaver_count_contigsClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$workspace_name is a seaver_count_contigs.workspace_name
$contigset_id is a seaver_count_contigs.contigset_id
$return is a seaver_count_contigs.CountContigsResults
...
# Authentication: required
	...
	method => "seaver_count_contigs.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "seaver_count_contigs.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for seaver_count_contigs::seaver_count_contigsClient\n";
    }
    if ($sMajor == 0) {
        warn "seaver_count_contigs::seaver_count_contigsClient version is $svr_version. API subject to change.\n";
    }
}
...
package seaver_count_contigs::seaver_count_contigsClient::RpcClient;
